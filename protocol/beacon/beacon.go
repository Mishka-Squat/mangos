...
const (
	Self     = protocol.ProtoBeacon
	Peer     = protocol.ProtoBeacon
	SelfName = "beacon"
	PeerName = "beacon"
)

const defaultBeaconTime = time.Second
...
type socket struct {
	...
	beacons  map[uint32]*beacon    // contexts by survey ID
...
func (s *beacon) cancel(err error) {
...
func NewProtocol() protocol.Protocol {
	s := &socket{
		...
		beacons:  make(map[uint32]*beacon),
