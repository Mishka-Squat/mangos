...
// Package udp implements the UDP transport for mangos. To enable it simply
// import it.
package udp
...
func (l *listener) Listen() (err error) {
	...
	l.l, err = l.lc.Listen(context.Background(), "udp", l.addr)
	if err != nil {
		return
	}
	l.bound = l.l.Addr()
...
func (t udpTran) NewDialer(addr string, sock mangos.Socket) (transport.Dialer, error) {
	var err error
...
func (t udpTran) NewListener(addr string, sock mangos.Socket) (transport.Listener, error) {
	var err error
	l := &listener{
		proto:  sock.Info(),
